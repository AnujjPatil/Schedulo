...
export const AddProjectMilestoneModal = () => {
  const { isOpen, onClose, type, data } = useModal();
  const router = useRouter();

  const isModalOpen = isOpen && type === "addProjectMilestone";
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      await axios.post(`/api/servers/${serverId}/projects/${project?.id}/milestones`, values);
      ...
      <DialogContent className="bg-white text-black p-0 overflow-hidden">
        <DialogHeader className="pt-8 px-6">
          ...
          <DialogDescription className="text-center text-zinc-500">
            Add a milestone to the project &quot;{project?.name}&quot;
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
